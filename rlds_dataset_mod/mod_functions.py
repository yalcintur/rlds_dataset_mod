...
class Downsample10x(TfdsModFunction):
    """
    A TfdsModFunction that downsamples episodes by a factor of 10.
    """
    ...
    @classmethod
    def mod_dataset(cls, ds: tf.data.Dataset) -> tf.data.Dataset:
        ds = ds.enumerate()  # (index, episode)
        ds = ds.filter(lambda idx, episode: tf.equal(tf.math.mod(idx, 10), 0))
        ds = ds.map(lambda idx, episode: episode)
        return ds
